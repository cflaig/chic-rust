...
impl ChessBoard {
    ...
    pub fn generate_legal_moves(&self, guess_of_best_move: Option<Move>) -> LazySortedMoves<Move> {
    ...
    pub fn generate_legal_moves_from_field(&self, field: ChessField) -> Vec<(i32, Move)> {
        let mut legal_moves = Vec::new();

        // Generate all pseudo-legal moves
        let pseudo_moves = self.generate_pseudo_moves_from_position(field.row, field.col);
    ...
    pub fn generate_legal_capture_moves(&self) -> LazySortedMoves<Move> {
        let mut legal_moves = Vec::new();

        for (score, mv) in self.generate_capture_moves() {
