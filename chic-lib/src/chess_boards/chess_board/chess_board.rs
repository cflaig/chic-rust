...
impl ChessBoard {
    ...
    pub fn from_fen(fen: &str) -> Result<Self, String> {
        fen::from_fen(fen).map(|mut board| {
            ...
            for (i, _pos) in piece_indexes.iter().enumerate() {
...
impl ChessBoard {
    ...
    pub fn all_pieces_with_coordinates<'a>(&'a self) -> impl Iterator<Item = (ChessField, &'a Piece)> {
        let piece_position = self.white_pieces_positions[0..self.white_pieces[6] as usize]
            .iter()
    ...
    ) -> impl Iterator<Item = (ChessField, &'a Piece)> {
        let (pieces_positions, pieces) = match color {
            Color::White => (&self.white_pieces_positions, &self.white_pieces),
            Color::Black => (&self.black_pieces_positions, &self.black_pieces),
        };

        pieces_positions
            [pieces[get_piece_type_index(&piece_type)] as usize..pieces[get_piece_type_index(&piece_type) + 1] as usize]
            .iter()
        ...
        match piece {
            ...
            Square::Occupied(p) => {
                if p.kind == PieceType::Pawn
                    || matches!(
                        self.squares[mv.to.row as usize][mv.to.col as usize],
                        Square::Occupied(_)
                    )
                {
                ...
                if let Some(en_passant) = self.en_passant {
                    if mv.to == en_passant && p.kind == PieceType::Pawn {
                        ...
                        hash = zobrist.update_square(
                            hash,
                            self.squares[mv.from.row as usize][mv.to.col as usize],
                            mv.from.row,
                            mv.to.col,
                        );
                        self.squares[mv.from.row as usize][mv.to.col as usize] = Square::Empty;
                        self.remove_piece_from_piece_position(
                            ChessField {
                                row: mv.from.row,
                                col: mv.to.col,
                            },
                ...
                if p.kind == PieceType::King {
                    if mv.from.col == 4 && mv.to.col == 6 && mv.from.row == mv.to.row {
                        if self.castling_rights[if self.active_color == Color::White { 0 } else { 2 }] {
                            ...
                            hash = zobrist.update_square(
                                hash,
                                self.squares[mv.from.row as usize][rook_col],
                                mv.from.row,
                                rook_col as u8,
                            );
    ...
    fn update_piece_position(&mut self, mv: Move, piece: Piece) {
        ...
        if self.active_color == Color::White {
            self.white_pieces_positions
                [self.white_pieces[piece_index] as usize..self.white_pieces[piece_index + 1] as usize]
                .iter_mut()
                .filter(|f| **f == mv.from)
                .for_each(|f| *f = mv.to);
        } else {
            self.black_pieces_positions
                [self.black_pieces[piece_index] as usize..self.black_pieces[piece_index + 1] as usize]
                .iter_mut()
                .filter(|f| **f == mv.from)
                .for_each(|f| *f = mv.to);
        }
    ...
    fn insert_piece_from_piece_position(&mut self, field: ChessField, piece: Piece) {
        ...
        if self.active_color == Color::White {
            Self::insert_pieces_array_and_positions(
                field,
                piece_index,
                &mut self.white_pieces,
                &mut self.white_pieces_positions,
            );
        } else {
            Self::insert_pieces_array_and_positions(
                field,
                ...
                &mut self.black_pieces_positions,
            );
        }
    }

    fn insert_pieces_array_and_positions(
        field: ChessField,
        piece_index: usize,
        pieces: &mut [u8; 7],
        piece_position: &mut [ChessField; 16],
    ) {
    ...
    fn remove_piece_from_piece_position(&mut self, field: ChessField, piece: Piece) {
        ...
        if piece.color == Color::White {
            Self::remove_pieces_array_and_positions(
                field,
                piece_index,
                &mut self.white_pieces,
                &mut self.white_pieces_positions,
    ...
    fn remove_pieces_array_and_positions(
        field: ChessField,
        piece_index: usize,
        pieces: &mut [u8; 7],
        piece_position: &mut [ChessField; 16],
    ...
    pub fn is_square_attacked_by_color(&self, row: u8, col: u8, opponent_color: Color) -> bool {
        ...
        for i in position[indexes[get_piece_type_index(&PieceType::Knight)] as usize
            ..indexes[get_piece_type_index(&PieceType::Knight) + 1] as usize]
            .iter()
        {
    ...
    fn get_piece_position_data_structure(&self, color: Color) -> (Vec<ChessField>, Vec<u8>) {
        ...
        for p in [
            ...
            self.find_piece_position_position_by_scanning(Piece { color, kind: p })
                .iter()
                .for_each(|f| {
...
mod tests {
    ...
    fn assert_piece_position_arrays(
        indexes: Vec<u8>,
        position: Vec<ChessField>,
        expected_position: Vec<ChessField>,
        ...
        for i in 0..indexes.len() - 1 {
            ...
            let expected_piece_positions = expected_position
                [expected_indexes[i] as usize..expected_indexes[i + 1] as usize]
                .to_vec()
                .sort();
            assert_eq!(
                piece_positions, expected_piece_positions,
                "failed at index {}, {}",
                i, msg
            );
        }
    ...
    fn test_piece_position() {
        ...
        let newboard = ChessBoard::from_fen("r3k1N1/1b6/n7/7n/1p6/Q7/2P3N1/4K2R b Kq - 0 1").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());

        board.make_move(Move::from_algebraic("e8c8"));
        let newboard = ChessBoard::from_fen("2kr2N1/1b6/n7/7n/1p6/Q7/2P3N1/4K2R w K - 1 2").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());

        board.make_move(Move::from_algebraic("c2c4"));
        let newboard = ChessBoard::from_fen("2kr2N1/1b6/n7/7n/1pP5/Q7/6N1/4K2R b K c3 0 2").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());

        board.make_move(Move::from_algebraic("b4c3"));
        let newboard = ChessBoard::from_fen("2kr2N1/1b6/n7/7n/8/Q1p5/6N1/4K2R w K - 0 3").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());

        board.make_move(Move::from_algebraic("e1g1"));
        let newboard = ChessBoard::from_fen("2kr2N1/1b6/n7/7n/8/Q1p5/6N1/5RK1 b - - 1 3").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());
        ...
        let newboard = ChessBoard::from_fen("2kr2N1/1b6/n7/7n/5N2/Q7/8/2r2RK1 w - - 0 5").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());

        let mut board =
            ChessBoard::from_fen("r3k2r/p1ppqpb1/Bn2pnp1/3PN3/4P3/2p2Q1p/PPPB1PPP/R3K2R w KQkq - 0 2").unwrap();
        board.make_move(Move::from_algebraic("e5f7"));
        let newboard =
            ChessBoard::from_fen("r3k2r/p1ppqNb1/Bn2pnp1/3P4/4P3/2p2Q1p/PPPB1PPP/R3K2R b KQkq - 0 2").unwrap();
        assert_piece_position(&board, &newboard, "".to_string());
    }
